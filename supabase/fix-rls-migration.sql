...
-- The self-referencing subquery policies cause 500 errors.
-- Replace with SECURITY DEFINER helper function.

-- 1. Helper function to check super_admin (bypasses RLS via SECURITY DEFINER)
...
-- 2. Drop problematic self-referencing policies
...
-- 3. Recreate with security definer helper (no recursion)
...
-- 4. Ensure admin user is seeded
INSERT INTO admin_users (id, email, full_name, role)
SELECT id, email, 'Admin', 'super_admin'
FROM auth.users
WHERE email = 'admin@aksharkala.com'
ON CONFLICT (id) DO NOTHING;

-- 5. Ensure activity_log entity_type includes 'sections'
...
-- 6. Ensure sections table exists (if it somehow got lost)
...
-- Seed default sections if they don't already exist
...
-- Ensure RLS is enabled on sections table
ALTER TABLE sections ENABLE ROW LEVEL SECURITY;

-- Public read: anyone can read active sections (no auth required)
...
DROP POLICY IF EXISTS "Admin users can manage sections" ON sections;
CREATE POLICY "Admin users can manage sections"
  ON sections FOR ALL
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

-- Ensure activity_log entity_type includes 'sections'
ALTER TABLE activity_log DROP CONSTRAINT IF EXISTS activity_log_entity_type_check;
ALTER TABLE activity_log ADD CONSTRAINT activity_log_entity_type_check
  CHECK (entity_type IN ('product', 'inquiry', 'order', 'settings', 'sections'));
